...
const PdtContainer = () => {
  ...
  const { data: categories }: { data: Type } = useSWR(
  ...
  useEffect(() => {
    if (data?.data) {
      if (categorySelected == "") {
        ...
        setProducts(
          data?.data.filter(
            (product: { type: string; category: string; altTag: string }) =>
              product.type === typeSelected &&
  ...
  const currentType = categories?.data.find(
    (item) => item.type === typeSelected,
  );

  console.log(currentType?.pageDescription);

  return (
    <section className="">
      <div className="container ">
        <div className="pt-16 pb-[70px] xl:pb-[120px]">
          <h1 className="text-2xl text-black mb-8 md:mb-12 leading-[1.3]">
            {categorySelected == ""
              ? typeSelected + " lighting products"
              : categorySelected}
          </h1>
                      ...
                      <div className="pdtcontainer min-h-[300px] md:min-h-[300px] xl:min-h-[435px] h-full border relative overflow-hidden cursor-pointer group">
                        ...
                        >
                          <p className="text-25 text-black  group-hover:text-white transition-500 mt-5 mb-8">
                            {product?.name.includes("W/m")
                              ? product?.name
                                  .split(" ")
                                  .filter((item) => item !== "W/m")
                                  .join(" ")
                                  .toUpperCase() + " W/m"
                              : product?.name
                                  .split(" ")
                          ...
                          >
                            <div className="flex justify-between ">
                              ...
                              <p className="text-sm md:text-[14px] lg:text-sm">
                                {product?.wattage}
                              </p>
                            </div>
                            <div className="flex justify-between  ">
                              {" "}
                              <p className="group-hover:text-white text-sm md:text-[14px] lg:text-sm">
                                Lumen
                              </p>{" "}
                              <p className="text-sm md:text-[14px] lg:text-sm">
                                {product?.lumen}
        ...
        {currentType?.pageTitle && (
          <div className="flex flex-col gap-[40px] border-t border-black/10 pt-[80px] xl:pt-[150px] pb-[70px] xl:pb-[120px]">
          <h2 className="text-2xl text-black leading-[1.16] tracking-[-0.01em] max-w-[28ch]">
            {currentType?.pageTitle}
          </h2>
          <p
            className="text-sm leading-[1.39] text-[#555555]"
            dangerouslySetInnerHTML={{
              __html: currentType?.pageDescription || "",
            }}
          />
        </div>
        )}

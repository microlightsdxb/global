...
//   industryData: {
//     data: { name: string }[];
//     setIndustrySelected: (name: string) => void;
//     industrySelected: string;
//   };
//   locationData: {
//     data: { name: string }[];
//     setLocationSelected: (name: string) => void;
...
//       setIndustryOptions([
//         ...industryData?.data?.map((item: { name: string }) => ({
//           value: item.name,
//           label: item.name,
//         })),
//       ]);
...
//       setLocationOptions([
//         ...locationData?.data?.map((item: { name: string }) => ({
//           value: item.name,
//           label: item.name,
//         })),
//       ]);
...
//                     src={assets.iconback}
//                     alt=""
//                     className="text-[22px] text-[#7D7D7D] group-hover:scale-125 transition-all ease-in-out duration-500"
//                   ></Image>
//                 </button>
//               </div>
//             </div>
...
//   return (
//     <div
//       className="relative w-full border-b  mt-1 flex justify-between"
//       ref={dropdownRef}
//     >
//       <div onClick={toggleDropdown} className="w-full">
//         <div className="bg-transparent border-b border-none text-white py-1 px-3 cursor-pointer select-none">
//           <div className="flex gap-3">
//             {selectedLocations.map((item, index) => (
//               <div
//                 className="px-1 bg-gray-500 flex gap-1 items-center"
//                 key={index}
//               >
//                 {item}
//                 <IoIosCloseCircle
...
//               <div
//                 key={index}
//                 className="flex gap-3 items-center px-3 w-full hover:text-white hover:bg-[#333] hover:cursor-pointer"
...
//         />
//       ) : (
//         <FaAngleDown className="text-[28px] pt-2 text-[rgb(204, 204, 204)]" />
//       )}
//     </div>
//   );
// };
...
const Filter = ({
  ...
  setIndustrySelected: (name: string) => void;
  // keeping these in props signature so parent doesn't break
  ...
  return (
    <section className="pt-100 pb-[50px]">
      <div className="container">
        <motion.div
          initial={{ opacity: 0, x: -50 }}
          whileInView={{ opacity: 1, x: 0 }}
          ...
          viewport={{ once: true, amount: 0.5 }}
        >
          <h1 className="text-2xl text-black mb-[40px] md:mb-[60px] 3xl:mb-[80px] leading-[1.3]">
            Lighting Projects Portfolio Across the UAE
          </h1>
        </motion.div>
        <motion.div
        ...
        >
          <div
            className="relative border-b border-black/20 overflow-x-auto"
            style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
          >
            <div className="flex gap-[40px] xl:gap-[70px] w-max md:w-auto">
              {industryData?.data?.map((item, index) => (
                <button
                  key={index}
                  type="button"
                  onClick={() => setIndustrySelected(item.name)}
                  className="relative text-sm text-black pb-[20px] cursor-pointer whitespace-nowrap"
                >
                  {item.name}
                  {industryData.industrySelected === item.name && (

...
const TypePage = () => {
  const [typeList, setTypeList] = useState<
    { _id: string; type: string; image: string; hoverImage: string; pageTitle: string; pageDescription: string }[]
  >([]);
  ...
  const handleFetchCategory = async () => {
    try {
      const response = await fetch(
        `/api/admin/product/category?typeId=${typeId}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleAddType = async () => {
    try {
      const response = await fetch("/api/admin/product/type", {
        method: "POST",
        body: JSON.stringify({ type, image, hoverImage, pageTitle, pageDescription }),
      });
      if (response.ok) {
        const data = await response.json();
        alert(data.message);
  ...
  const handleEditType = async (id: string) => {
    try {
      const response = await fetch(`/api/admin/product/type`, {
        method: "PATCH",
        body: JSON.stringify({ id, type, image, hoverImage, pageTitle, pageDescription }),
      });
      if (response.ok) {
        const data = await response.json();
        alert(data.message);
  ...
  const handleSavePageContent = async (id: string) => {
  try {
    const response = await fetch(`/api/admin/product/type`, {
      method: "PATCH",
      body: JSON.stringify({ id, pageTitle, pageDescription }),
    });
    if (response.ok) {
      const data = await response.json();
      alert(data.message);
  ...
  const getTaskPos = (id: number | string) =>
    categoryList.findIndex((item: { _id: string }) => item._id == id);
    ...
    <div className="h-screen grid grid-cols-1 gap-5">
      <div className="h-full w-full p-5 shadow-md border-gray-300 rounded-md bg-white">
        ...
        <div className="mt-2 flex flex-col gap-2 h-[80%] overflow-y-auto">
          {typeList.map((item) => (
            <div
              className="flex justify-between border p-3 items-center rounded-md shadow-md hover:shadow-lg transition-all duration-300"
              key={item._id}
            >
                      ...
                      <div className="flex flex-col gap-2 mt-5">
                        {!reorderMode &&
                          categoryList.map((item) => (
                            <div
                              key={item._id}
                              className="flex justify-between border p-3 items-center rounded-md text-[16px]"
                            >
                              {item.name}
                              <div className="flex gap-5">
                                <Dialog>
  ...
  <SheetTrigger
    onClick={() => {
      ...
      setPageDescription(item.pageDescription || "");
    }}
  >
    <MdEditDocument />
  </SheetTrigger>
  <SheetContent side="right" className="w-[600px] sm:w-[700px] overflow-y-auto px-4">
    <SheetHeader>
      <SheetTitle>Page Content — {item.type}</SheetTitle>
    </SheetHeader>
    <div className="flex flex-col gap-4 mt-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <TinyEditor setNewsContent={setPageDescription} newsContent={pageDescription} />
      </div>
      <Button onClick={() => handleSavePageContent(editingTypeId)}>Save</Button>
    </div>
  </SheetContent>
</Sheet>
                ...
                <Dialog>
                  ...
                  <DialogContent>
                    ...
                    <DialogClose
                      className="bg-black text-white px-2 py-1 rounded-md"
                      onClick={() => handleEditType(item._id)}
                    >
                      Save
                    </DialogClose>
                  </DialogContent>
